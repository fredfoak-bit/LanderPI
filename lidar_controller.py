#!/usr/bin/env python2
# encoding: utf-8
# (lidar obstacle avoidance and lidar following function)
import os
import math
import time
import rclpy
...
if 'Pro' in os.environ.get('MACHINE_TYPE'):
    MAX_SCAN_ANGLE = 120  # degree(the scanning angle of Lidar. The covered part is always eliminated)
else :
    MAX_SCAN_ANGLE = 240
...
class LidarController(Node):
    def __init__(self, name):
        ...
        self.create_service(SetFloat64List, '~/set_param', self.set_parameters_srv_callback)  # (set parameter)
            ...
            if 'Acker' not in self.machine_type:
                if self.running_mode == 1 and self.timestamp <= time.time():
                    ...
                    if len(min_dist_left_) > 1 and len(min_dist_right_) > 1:
                        ...
                        if min_dist_left <= self.threshold and min_dist_right > self.threshold:  # (there is obstacle at left)
                            twist.linear.x = self.speed / 6  # (keep the forward velocity as low as possible when turning)
                            max_angle = math.radians(90)  # 转90度(turn 90 degree)
                            w = self.speed * 6.0  # (keep the turning speed as high as possible)
                            twist.angular.z = -w
                        ...
                        elif min_dist_left > self.threshold and min_dist_right <= self.threshold:  # (there is obstacles at right)
                            twist.linear.x = self.speed / 6  # (keep the forward velocity as low as possible when turning)
                            max_angle = math.radians(90)  # turn 90 degrees
                            w = self.speed * 6.0  # (keep the turning speed as high as possible)
                            twist.angular.z = w
                    ...
                    if len(dist_) > 0:
                        ...
                        if dist < self.threshold and abs(math.degrees(angle)) > 5:  # (control the left and right)
                            if self.lidar_type != 'G4':
                        ...
                        if dist < self.threshold and abs(0.2 - dist) > 0.02:  # (control the front and back)
                ...
                if self.running_mode == 1 and self.timestamp <= time.time():
                    ...
                    if len(min_dist_left_) > 1 and len(min_dist_right_) > 1:
                        ...
                        if self.last_act == 3: 
                            ...
                            twist.angular.z = -twist.linear.x/0.1 # # 0.1m
                            self.last_act = 4
                            self.mecanum_pub.publish(twist)
                        ...
                        elif (min_dist_left <= self.threshold and min_dist_right > self.threshold):  # (there is obstacle at left)
                            twist.linear.x = self.speed
                   ...
                   # (the merged distance data from right half counterclockwise to the left half)
                    ...
                    if len(dist_) > 1:
                        ...
                        if dist < 2 and abs(0.5 - dist) > 0.02:  # (control the front and back)
                            self.pid_dist.SetPoint = 0.5
                            self.pid_dist.update(dist)
                            twist.linear.x = common.set_range(-self.pid_dist.output, -0.35, 0.35)
                        else:
                            self.pid_dist.clear()
                        if dist < 2 and abs(math.degrees(angle)) > 5:  # (control the left and right)
                            if self.lidar_type != 'G4':
                                self.pid_yaw.update(-angle)
