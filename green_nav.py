...
# Lidar scanning angle used for obstacle checks (degrees)
MAX_SCAN_ANGLE = 240
...
class LineFollower:
    ...
    def __init__(self, color, node):
        ...
        # Correct weighted-sum normalisation
        self.weight_sum = sum(r[-1] for r in self.rois)
    ...
    def __call__(self, image, result_image, threshold, color=None, use_color_picker=True):
        ...
        # Original ascamera quirk: extra width
...
class GreenLineFollowingNode(Node):
    ...
    def __init__(self, name: str):
        ...
        # Stop / avoidance thresholds
        self.stop_threshold = float(self.declare_parameter('stop_threshold', 0.15).value)

        # --- Obstacle avoidance state ---
        # When LiDAR sees an obstacle within stop_threshold, we schedule a short
        # turning manoeuvre that overrides line-following.
        self.avoid_active_until = 0.0       # wall-clock time until which avoidance twist is valid
        self.avoid_twist = Twist()          # last avoidance command
        self.avoid_last_act = 0             # remembers last turn direction (for alternating)
        self.avoid_timestamp = 0.0          # next time we’re allowed to schedule a new avoidance move
        self.avoid_speed = 0.2              # base speed during avoidance (m/s)
        ...
        # LAB config and camera parameters
        ...
        # Handle auto-declared params without double-declare crashes.
        ...
        self.log_debug(
            f"Debug logging enabled. DEPTH_CAMERA_TYPE={self.camera_type}, "
            f"using LAB key={self.lab_lookup_type}, LIDAR_TYPE={self.lidar_type}, "
            f"MACHINE_TYPE={self.machine_type}"
        )
        self.log_debug(
            f"Stop threshold set to {self.stop_threshold} meters "
            f"(adjust with parameter stop_threshold)"
        )
    ...
    def _image_watchdog(self):
        ...
        if self.last_image_ts is None:
            self.log_debug(
                "Waiting for first image on topic: "
                + self.image_topic
                + " (override with parameter image_topic)"
            )
        elif now - self.last_image_ts > 5.0:
            self.log_debug(
                f"No images received for {now - self.last_image_ts:.1f}s on {self.image_topic} "
                "(override with parameter image_topic)"
            )
    ...
    def enter_srv_callback(self, request, response):
        ...
        with self.lock:
            ...
            self.avoid_timestamp = 0.0
            self.avoid_last_act = 0

            self.log_debug(
                "Entering green_nav: reset PID and thresholds; "
            ...
            if self.lidar_sub is None:
                ...
                # Pitch robot arm up to see the green beacon / track
                set_servo_position(
                    self.joints_pub,
                    1,
                    ((10, 200), (5, 500), (4, 90), (3, 150), (2, 820), (1, 500)),
                )
    ...
    def set_running_srv_callback(self, request, response):
        ...
        with self.lock:
            ...
            if not self.is_running:
                self.mecanum_pub.publish(Twist())
            self.log_debug(
                f"set_running called: is_running={self.is_running}, stop={self.stop}, "
                f"searching_for_green={self.searching_for_green}"
            )
        response.success = True
    ...
    # ------------------------------------------------------------------ #
    # LiDAR callback: obstacle avoidance
    # ------------------------------------------------------------------ #

    def lidar_callback(self, lidar_data: LaserScan):
        # If lidar_type isn't set, don't try to interpret the scan.
        if self.lidar_type is None:
            return

        # Compute left/right sectors similarly to original logic
        ...
        min_dist_left = float(min_dist_left_.min())
        min_dist_right = float(min_dist_right_.min())

        # --- Emergency hard stop if something is extremely close ---
        emergency_threshold = 0.05  # ~5 cm
        if min_dist_left < emergency_threshold or min_dist_right < emergency_threshold:
            with self.lock:
                self.stop = True
                self.avoid_active_until = 0.0
            self.log_debug(
                f"Emergency stop: left={min_dist_left:.2f}, right={min_dist_right:.2f}, "
        ...
        with self.lock:
            # If obstacle is within stop_threshold, schedule an avoidance manoeuvre
            if min_dist_left < self.stop_threshold or min_dist_right < self.stop_threshold:
                # Don’t reschedule if we’re still in the middle of a previous avoidance move
                if now < self.avoid_timestamp:
                    return

                twist = Twist()
                # Small forward crawl plus a turn
                twist.linear.x = self.avoid_speed / 6.0
                w = self.avoid_speed * 6.0

                # Both sides blocked: alternate left/right
                if min_dist_left < self.stop_threshold and min_dist_right < self.stop_threshold:
                    if self.avoid_last_act in (0, 2):
                        twist.angular.z = -w  # turn right this time
                        self.avoid_last_act = 1
                    else:
                        twist.angular.z = w   # turn left this time
                        self.avoid_last_act = 2
                # Only left side blocked → turn right
                elif min_dist_left < self.stop_threshold:
                    twist.angular.z = -w
                    self.avoid_last_act = -1
                # Only right side blocked → turn left
                else:
                    twist.angular.z = w
                    self.avoid_last_act = 1

                duration = 0.6  # seconds to keep this avoidance command; tune as needed
                self.avoid_twist = twist
                self.avoid_active_until = now + duration
                self.avoid_timestamp = self.avoid_active_until
                self.stop = False  # we’re actively avoiding, not freezing

                self.log_debug(
                    f"Avoiding obstacle: left={min_dist_left:.2f}, right={min_dist_right:.2f}, "
                    f"turn_z={twist.angular.z:.2f}, duration={duration}"
                )
            else:
                # Clear path for several frames → cancel avoidance and stop flags
                self.count += 1
                if self.count > 5:
                    self.count = 0
                    self.stop = False
                    self.avoid_active_until = 0.0
                    self.log_debug(
                        f"Lidar clear: left={min_dist_left:.2f}, right={min_dist_right:.2f}"
                    )

    # ------------------------------------------------------------------ #
    # Image callback: line following + avoidance override
    # ------------------------------------------------------------------ #
    ...
    def image_callback(self, ros_image: Image):
        ...
        with self.lock:
            ...
            # Base forward speed when line following
            ...
            if lab_config is None and 'ascamera' in lab_map:
                ...
                self.log_debug("Falling back to LAB config key: ascamera")
            ...
            if lab_config is None:
                self.get_logger().error(
                    "LAB config missing for selected color; cannot proceed."
                )
                return
            ...
            # --- Priority 1: emergency stop from LiDAR ---
            if self.stop:
                cmd = Twist()

            # --- Priority 2: active avoidance manoeuvre from LiDAR ---
            elif now < self.avoid_active_until and self.is_running:
                cmd = self.avoid_twist

            # --- Priority 3: normal green-line following ---
            elif deflection_angle is not None and self.is_running:
            ...
            # --- Priority 4: searching spin (when we lost the line) ---
            elif self.is_running and self.searching_for_green:
                twist.linear.x = 0.0
                twist.angular.z = self.search_angular_speed
                cmd = twist

            else:
                # No line / not running / nothing to do → clear PID and stop
                self.pid.clear()
                cmd = Twist()

            # Publish if running, or if we need to enforce a stop/avoidance
            if self.is_running or self.stop or (now < self.avoid_active_until):
            ...
            if self.frame_count % self.log_interval == 0:
                ...
                pid_output_str = (
                    f"{pid_output:.3f}" if isinstance(pid_output, (int, float)) else "n/a"
                )
                self.log_debug(
                    f"Frame {self.frame_count}: running={self.is_running}, stop={self.stop}, "
                    f"searching={self.searching_for_green}, deflection={deflection_angle}, "
                    f"pid_out={pid_output_str}, avoid_active={now < self.avoid_active_until}"
                )
